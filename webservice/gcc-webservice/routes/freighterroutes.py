...
def add_freighter():
    ...
    if request.method == 'POST': 
        # save details
        mongo.db.freighters.insert({'name': _name, 'type': _type, 'image_url': _image_url, 'system': _system, 'address': _address, 'area': _area, 'class': _ship_class, 'slots': _slots})
...
def update_freighter():
    ...
    _ship_class = _json['class']
    _slots = _json['slots']
    mongo.db.freighters.update_one({'name': _name}, {'$set': {'type': _type, 'image_url': _image_url, 'system': _system, 'address': _address, 'area': _area, 'class': _ship_class, 'slots': _slots}})
